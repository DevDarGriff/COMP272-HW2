 ...
 */
 ...
 public class BinaryTree {
     ...
     private void replaceValueHelper(Node node, int oldVal, int newVal) {
         ...
         if (node.data == oldVal) {                             //if node's data == old val;
            node.data = newVal;                                 //update node's value to new
         }

         replaceValueHelper(node.left, oldVal, newVal);         //recursively replace left node val w/ new val
         replaceValueHelper(node.right, oldVal, newVal);        //recursively replace right node val w/ new val
     ...
     private int findMinHelper(Node node) {
         ...
         // ADD YOUR CODE HERE -- USE DEPTH FIRST SEARCH OF
         // BINARY TREE (WHICH IS BASED ON RECURSION)
         if (node == null) {                                        //if node is null; return Integer.MAX_VALUE
            return Integer.MAX_VALUE;
         }

         int value = node.data;
         int leftMin = findMinHelper(node.left);                    //recursively assign left node's value in variable
         int rightMin = findMinHelper(node.right);                  //recursively assign right node's value in variable

         return Math.min(value, Math.min(leftMin, rightMin));       //return min value between current, left, & right nodes
     ...
     private int nodesGTHelper(Node node, int val) {
         ...
         int count = 0;                                     //declare & initialize count variable to 0

         if (node.data > val) {                             //if current node's data > val; count = 1
            count = 1;
         } 
            count += nodesGTHelper(node.left, val);         //recursively increment count of nodes in left subtree
            count += nodesGTHelper(node.right, val);        //recursively increment count of nodes in right subtree

         return count;                                      //return node count in tree
     }
     ...
     private int[] averageHelper(Node n) {
         ...
         int sum = 0;                                   //int variables to store int & count
         int count = 0;

         if (n == null) {                               //if n node is null, return empty int[]
            return new int[]{0, 0};
         }

         int[] left = averageHelper(n.left);            //recursion into left subtree; stored in int[] 
         int[] right = averageHelper(n.right);          //recursion into right subtree; stored in int[]

         sum += left[0] + right[0] + n.data;            //sum += left's sum + right's sum + current's sum
         count += left[1] + right[1] + 1;               //count += left's count + right's count + current's count
        
         return new int[]{sum, count};                  //return new int[] containing sum & count values
     }
 }
